...
其它可选变量（都有合理默认值，一般无需设置）：

| 变量 | 默认 | 说明 |
|---|---|---|
| `DSH_ZEN_MODELS_TTL_MS` | `600000`（10 分钟） | 实时模型目录的刷新 TTL |
| `DSH_ZEN_STREAM_IDLE_TIMEOUT_MS` | `300000`（5 分钟） | 流式"死连接"判定：连续多久收不到任何字节才中止。`0` = 关闭看门狗 |
| `DSH_ZEN_NONSTREAM_TIMEOUT_MS` | `600000`（10 分钟） | 非流式请求墙钟上限。正整数=毫秒；`0` / `unlimited` / `infinity` / `none` = **不主动超时**（只等你手动取消） |

**关于等待时长**：模型"雷霆大思考"时到底能等多久？

- **流式路径没有总时长上限**——只有"静默"上限（即上面的 idle 看门狗）。只要上游还在吐字节，等多久都行；这是最健康的模型。
- **非流式路径是黑盒**——网关会把整段生成缓冲完才发响应头，客户端拿不到任何进度，无法区分"模型在苦思"和"连接已挂死"，只能用时间估算。实测 union-alpha 吞吐约 **23 tok/s**，我们按保守下界 10 tok/s 反推预算并设硬上限。
- **会不会无限等？** 默认不会。因为满额 131072 tok 按实测吞吐要 ≈95 分钟，真等下去会把界面冻住数小时，而连接挂死时也要陪跑同样时长。所以默认封顶 10 分钟，超时后如实报错、由你决定是否重试（你也可随时手动取消）。
- **想一直等**：设 `DSH_ZEN_NONSTREAM_TIMEOUT_MS=unlimited`。代价是挂死连接也会一直挂着，只能靠你手动取消。
