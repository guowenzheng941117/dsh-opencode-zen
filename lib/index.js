...
const DEFAULT_MODELS = [
  ...
  { id: 'mimo-v2.5-free', name: 'MiMo 2.5 (Free)', contextWindow: 200000, description: 'OpenCode Zen 免费档（小米 MiMo 2.5，支持视觉）', input: ['text', 'image'] },
...
/**
 * zen 免费档的"入场券"：网关要求匿名请求携带 session 头，否则一律
 * `400 MissingSessionID — OpenCode's free tier can only be used in OpenCode`。
 * 这不是鉴权（值随意、无需签名），而是 opencode CLI 用来做归属/attribution
 * 与后端亲和的标记：见 opencode 的 llm.ts —— 每次请求都带上
 * `x-session-id: <sessionID>`（子会话另有 `x-parent-session-id`）。
 * 我们从 GenerateOptions.sessionId 取真实会话 id，缺省时回退到进程级稳定
 * 兜底 id（同一进程内固定，保证一轮对话的多续跑/多路视觉请求归因一致）。
 */
const FALLBACK_SESSION_ID = `ses_${randomUUID().replace(/-/g, '').slice(0, 24)}`

/** 把宿主会话 id（可能是任意品牌字符串）规整成网关可接受的 wire 形态 */
function normalizeSessionId(raw) {
  if (typeof raw !== 'string') return null
  const id = raw.trim()
  if (!id) return null
  // 网关只要求非空；剔除换行/控制字符以免拆坏 HTTP 头
  const cleaned = id.replace(/[^\x20-\x7e]/g, '-')
  return cleaned || null
}

/**
 * 组装 zen 请求头。所有发往 zen 的请求（主对话 / 视觉旁路 / 目录拉取）
 * 都必须经过这里，确保 session 头一处不漏——漏一个就是整条链路 400。
 */
function zenHeaders(sessionId) {
  return {
    ...
    'x-session-id': normalizeSessionId(sessionId) || FALLBACK_SESSION_ID,
  }
}
...
function loadSpecCache() {
  try {
    if (existsSync(MODELS_DEV_CACHE)) {
      ...
      if (c && c.map && Date.now() - (c.fetchedAt || 0) < LIVE_TTL_MS) return c.map
    }
  } catch { /* ignore */ }
  return null
}
function saveSpecCache(map) {
  try {
    ...
    writeFileSync(MODELS_DEV_CACHE, JSON.stringify({ fetchedAt: Date.now(), map }))
  } catch { /* best effort */ }
}
...
async function ensureLiveModels(ctx) {
  ...
  _liveFetching = (async () => {
    ...
    if (!specMap) {
      try {
        ...
        for (const [mid, m] of Object.entries(models)) {
          if (m && typeof m === 'object' && /free/i.test(m.id || mid)) map[m.id || mid] = m
        }
    ...
    if (!candidateIds && specMap) candidateIds = Object.keys(specMap).filter((id) => /free/i.test(id))
    ...
    for (let attempt = 0; attempt < MAX_REQUEST_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          // 免费档归属闸门：网关认为请求不是从 opencode 发出的（通常意味着
          // x-session-id 没送到——被代理剥离、或上游改了头名）。给出可操作
          // 的提示，而不是让用户对着一句广告语干瞪眼。
          if (/MissingSessionID|free tier can only be used/i.test(raw)) {
            const e = new Error(
              `OpenCode Zen rejected the request at the attribution gate (MissingSessionID): the session header did not reach the gateway. ` +
              `This plugin sends "x-session-id" on every zen request; check for a proxy/middleware stripping custom headers. Raw: ${raw.slice(0, 200)}`,
            )
...
module.exports = { apply, inject, name, OpenCodeZenAdapter, StreamTranslator, buildContinuationBody, truncationBackoff, serializeMessages, multimodalParts, visionDescribe, visionDescribeBackground, VISION_DESC_CACHE, inputModalitiesOf, PROVIDER, MODELS, resolveApiKey, zenHeaders, normalizeSessionId }