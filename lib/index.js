...
/**
 * 已证实不再免费/已下线的模型：无论在哪个数据源（zen /models、models.dev、静态
 * models.json）出现都强制剔除。zen 撤下免费档时常是先从官网价格表移除、id 仍留在
 * /models 目录里（如曾经的 deepseek-v4-flash-free / muse-spark-1.2-contributor-free），
 * 或干脆从目录消失（如 hy3-free、laguna-s-2.1-free）；旧磁盘规格缓存（TTL 最长
 * 10 分钟）也可能把已下线模型继续带回选择器。此处是最后闸门。
 * 2026-09-17 对照官网 pricing 核实：hy3-free / laguna-s-2.1-free 已下线，
 * deepseek-v4-flash-free / muse-spark-1.2-contributor-free 免费状态不再成立。
 */
...
/**
 * 原生收图模型：图片以 image 内容块**直传主请求**，不再经文字旁路。
 * 只对实测确认原生视觉可靠的模型开启——旁路架构（图不入主请求）对上游视觉
 * 端点不稳定的模型仍有价值，故不全局切换。
 * union-alpha 已实测：喂随机四象限色的图能准确报出每象限 RGB，且无图对照
 * 会明确拒答（证明是读像素而非猜测）。
 */
const NATIVE_IMAGE_MODELS = new Set(['union-alpha'])

/** 该模型是否直传原图（而非转文字描述）。models.json 的 nativeImage 优先。 */
function usesNativeImage(model) {
  if (!model) return false
  if (typeof model.nativeImage === 'boolean') return model.nativeImage
  return NATIVE_IMAGE_MODELS.has(model.id)
}
...
/** 静态/兜底条目过滤：`retired: true`（或排除名单命中）的模型绝不进入选择器 */
function isExcludedEntry(m) {
  return !m || EXCLUDED_MODEL_IDS.has(m.id) || m.retired === true
}
...
/** v3：新增 EXCLUDED_MODEL_IDS 退役闸门（成员筛选逻辑变更，须令旧缓存失效，
 *  否则缓存里残留的 hy3-free / laguna-s-2.1-free 等仍会参与成员判定）。 */
const SPEC_CACHE_VERSION = 3
...
function normalizeEntry(id, spec, staticEntry) {
  ...
  return {
    ...
    // 原生收图：models.json 可显式声明 nativeImage 覆盖内置名单
    // （true=直传原图；false=强制走文字旁路；缺省=按 NATIVE_IMAGE_MODELS）
    ...(s && typeof s.nativeImage === 'boolean' ? { nativeImage: s.nativeImage } : {}),
...
async function ensureLiveModels(ctx) {
  ...
  _liveFetching = (async () => {
    ...
    const staticMap = new Map(loadModels().filter((m) => !isExcludedEntry(m)).map((m) => [m.id, m]))
    _liveModels = candidateIds.filter((id) => !EXCLUDED_MODEL_IDS.has(id)).map((id) => normalizeEntry(id, specMap ? specMap[id] || null : null, staticMap.get(id)))
    if (!_liveModels.length) throw new Error('no free models from any source')
    ...
    return _liveModels || loadModels().filter((m) => !isExcludedEntry(m))
  }).finally(() => { _liveFetching = null })
  return _liveFetching
}

/** 当前生效的模型目录：实时优先，静态兜底（静态路径同样剔除已退役条目） */
function currentModels() {
  const live = _liveModels && _liveModels.length ? _liveModels : null
  if (live) return live
  return loadModels().filter((m) => !isExcludedEntry(m))
}
...
/**
 * 流式空闲看门狗阈值：连续多久收不到**任何字节**才判定连接已挂死。
 * 语义是"死连接检测"，不是"生成时长上限"——长思考期间上游可能长时间不发字节，
 * 若阈值过小会把正常的长回答误杀。
 * 官方内置 `@deepseek-ai/dsh-llm-deepseek` 的同名默认值是 **300000ms(5min)**
 * （该适配器同样面向 reasoning 型上游）；旧值 30s 比官方严 10 倍，风险偏高，
 * 故对齐官方默认。可用 `DSH_ZEN_STREAM_IDLE_TIMEOUT_MS` 覆盖（正整数毫秒；
 * `0` = 关闭看门狗，只依赖宿主取消）。
 */
const DEFAULT_STREAM_IDLE_TIMEOUT_MS = (() => {
  const raw = String(process.env.DSH_ZEN_STREAM_IDLE_TIMEOUT_MS ?? '').trim()
  if (raw === '0') return 0
  const n = Number(raw)
...
/**
 * 流式连接超时：语义是"首字节/响应头到达"的时间上限。
 * 实测流式请求 1s 内即可拿到响应头，45s 足够。
 */
const CONNECT_TIMEOUT_MS = 45000
/**
 * 非流式请求的响应头超时（独立一档，**不能沿用 CONNECT_TIMEOUT_MS**）。
 * 实测（2026-09-17）：非流式时网关把整段生成缓冲完才发响应头，
 * `fetch()` resolve 时刻 ≈ 整段生成耗时，而非首字节时间。沿用 45s 会让
 * 任何超过 45s 的正常长回答被判 "connect timeout"（union-alpha 长文即踩此坑）。
 *
 * 那么给多久？非流式**没有任何进度信号**，无法区分"模型在苦思"与"连接已挂死"，
 * 故只能用时间和吞吐来估算，而不是拍一个固定值：
 *   实测吞吐 ≈ 23.1 tok/s（union-alpha，4062 tok / 175.8s）。
 *   取保守下界 NONSTREAM_FLOOR_TPS=10 再乘安全系数，反推所需墙钟。
 * 注意 union-alpha 满额 max_tokens=131072 按实测吞吐需 ≈ 95 分钟——**不能真等这么久**：
 * 那会把会话 UI 冻住数小时，而挂死连接也会陪跑同样时长。
 * 故设硬上限 NONSTREAM_TIMEOUT_MAX_MS（默认 10 分钟，可用环境变量放宽/收紧）：
 * 超限即放弃并如实报错，由用户决定重试；用户也随时可主动取消（宿主 signal 贯穿到 fetch）。
 * 想"一直等"就把上限调大，但默认不做无限等待。
 */
const NONSTREAM_FLOOR_TPS = 10
const NONSTREAM_SAFETY_FACTOR = 1.5
const NONSTREAM_TIMEOUT_MIN_MS = 60000
/** 非流式墙钟硬上限。默认 10 分钟；DSH_ZEN_NONSTREAM_TIMEOUT_MS 可调：
 *  正整数=毫秒；"unlimited"/"0"=不主动超时（等宿主 signal 取消），
 *  受 MAX_TIMER_DELAY_MS(2147483647ms≈24.8天) 保护。 */
const NONSTREAM_TIMEOUT_MAX_MS = (() => {
  const raw = String(process.env.DSH_ZEN_NONSTREAM_TIMEOUT_MS ?? '').trim()
  if (/^(0|unlimited|infinity|none)$/i.test(raw)) return 2147483647
  const n = Number(raw)
  const v = Number.isFinite(n) && n > 0 ? n : 10 * 60 * 1000
  return Math.max(NONSTREAM_TIMEOUT_MIN_MS, Math.min(2147483647, v))
})()

/** 由 max_tokens 与保守吞吐下界反推非流式墙钟预算，并夹在 [min, max] 内。 */
function nonstreamTimeoutMs(maxTokens) {
  const budget = Number(maxTokens) > 0 ? Number(maxTokens) : DEFAULT_MAX_TOKENS
  const est = (budget / NONSTREAM_FLOOR_TPS) * 1000 * NONSTREAM_SAFETY_FACTOR
  return Math.min(NONSTREAM_TIMEOUT_MAX_MS, Math.max(NONSTREAM_TIMEOUT_MIN_MS, est))
}
...
/**
 * 原生收图路径：把 image 块按目标协议直接送进主请求（不换成文字描述）。
 * 与 multimodalParts（旁路转文字）相对；两者都在 tool-result 内层递归。
 * 适用前提：模型确实原生支持视觉（如 union-alpha）——此时直传像素严格优于
 * 有损的文字化：省一次往返、颜色/细节/版式精确保留。
 * 取字节仍复用宿主附件服务（loadImage），故像素预算/缩放/编码与旁路一致。
 */
async function nativeImageParts(blocks, loadImage, protocol) {
  const parts = []
  let text = ''
  const flushText = () => { const t = text.trim(); if (t) { parts.push({ type: 'text', text: t }) } text = '' }
  async function walk(bs) {
    for (const b of bs || []) {
      if (b.type === 'text') text += b.text
        ...
        try {
          const v = await loadImage(b.attachment)
          if (!v?.data) { parts.push({ type: 'text', text: '[image unavailable]' }); continue }
          const data = Buffer.from(v.data).toString('base64')
          const mediaType = v.mediaType || 'image/png'
...
async function serializeMessages(messages, systemPrompt, loadImage, visCtx) {
  ...
  for (const m of messages || []) {
    ...
    if (role === 'user' && loadImage && toolResults.length === 0 && hasImageDeep(m.content)) {
      // 原生收图优先（模型自己能看像素时，直传远优于有损文字化）
      if (visCtx?.nativeImage) {
        const parts = await nativeImageParts(m.content, loadImage, 'openai')
        if (parts.length > 0) { wire.push({ role: 'user', content: parts }); continue }
      }
    ...
    for (const r of toolResults) {
      // 工具结果里的图不走原生直传（实测模型对 tool_result 内的图块视而不见），
      // 一律经文字旁路；理由详见 serializeAnthropicMessages 同位置注释。
  ...
  for (const m of messages || []) {
    ...
    if (toolResults.length === 0) {
      if (loadImage && hasImageDeep(m.content)) {
        // 原生收图优先：模型自己看得见像素时，直传远优于有损文字化
        if (visCtx?.nativeImage) {
          const parts = await nativeImageParts(m.content, loadImage, 'anthropic')
          out.push({ role: 'user', content: parts.length ? parts : [{ type: 'text', text: text || '(no content)' }] })
          continue
        }
        const parts = await multimodalParts(m.content, loadImage, visCtx)
    ...
    if (text) blocks.push({ type: 'text', text })
    for (const r of toolResults) {
      // 注意：工具结果里的图**不走原生直传**，即便模型支持原生视觉。
      // 实测（union-alpha）：把图块塞进 tool_result.content 后上游虽返回 200，
      // 但模型明确表示看不到像素（"can't visually inspect that base64 payload"）、
      // 要求"直接附图"，四个象限颜色全部答错；同一张图作为**用户消息**直传则
      // 四象限全中。即工具结果里的原生图不被模型有效接收，故此处一律走文字旁路。
      let content
      if (loadImage && hasImageDeep(r.content)) {
        const parts = await multimodalParts(r.content, loadImage, visCtx)
...
class OpenCodeZenAdapter {
  ...
  async openStreamOnce(reqBody, options) {
    ...
    for (let attempt = 0; attempt < MAX_REQUEST_ATTEMPTS; attempt++) {
      ...
      // 超时预算按请求形态分档：流式=首字节上限；非流式=整段生成上限，
      // 由该请求的 max_tokens 与保守吞吐下界反推（见 nonstreamTimeoutMs）。
      const connectTimeoutMs = options.timeoutMs
        || (options.nonStreaming ? nonstreamTimeoutMs(reqBody?.max_tokens) : CONNECT_TIMEOUT_MS)
      const connectTimer = setTimeout(() => { state.selfAbort = 'connect'; controller.abort() }, connectTimeoutMs)
  ...
  async *stream(options) {
    ...
    // 视觉策略：默认走文字旁路（图不入主请求，隔离上游视觉端点故障）；
    // 对 NATIVE_IMAGE_MODELS 则直传原图——它原生看得见像素，直传省一次往返
    // 且无有损文字化。两种模式取字节都走同一个 loadImage（预算/缩放一致）。
    const nativeImage = usesNativeImage(found) || usesNativeImage({ id: model })
    const visCtx = { ctx: this.ctx, modelId: model, signal, sessionId, nativeImage }
    ...
    let translator
    if (useAnthropic) {
      const wire = await serializeAnthropicMessages(messages, system, loadImage, visCtx)
      ...
      const wireMessages = await serializeMessages(messages, system, loadImage, visCtx)
...
module.exports = { apply, inject, name, OpenCodeZenAdapter, StreamTranslator, AnthropicStreamTranslator, buildContinuationBody, buildAnthropicContinuationBody, anthropicMessageToEvents, describeRequest, extractDescribeText, nativeImageParts, truncationBackoff, serializeMessages, serializeAnthropicMessages, serializeAnthropicTools, parseAnthropicSse, multimodalParts, visionDescribe, visionDescribeBackground, VISION_DESC_CACHE, inputModalitiesOf, isFreeModelId, wireProtocolOf, usesNativeImage, ANTHROPIC_PROTOCOL_MODELS, NATIVE_IMAGE_MODELS, PROVIDER, MODELS, resolveApiKey, zenHeaders, normalizeSessionId, nonstreamTimeoutMs, NONSTREAM_TIMEOUT_MAX_MS }