...
/**
 * 非流式请求的响应头超时（单独一档，**不能沿用 45s**）。
 * 实测（2026-09-17）：非流式时网关会把整段生成缓冲完再发响应头——
 * 长回答实测 headers 与 body 同在 27.5s 才到达，即 fetch() resolve 时刻
 * ≈ 整段生成耗时。若沿用 CONNECT_TIMEOUT_MS，任何超过 45s 的正常长回答
 * 都会被误判为 "connect timeout" 而整轮失败（union-alpha 长文即踩此坑）。
 * 故该线按"整段生成总预算"给足，与 max_tokens 量级匹配。
 */
const NONSTREAM_TIMEOUT_MS = 240000
...
class OpenCodeZenAdapter {
  ...
  async openStreamOnce(reqBody, options) {
    ...
    for (let attempt = 0; attempt < MAX_REQUEST_ATTEMPTS; attempt++) {
      ...
      // 超时预算按请求形态分档：流式=首字节上限；非流式=整段生成上限（见常量注释）。
      const connectTimeoutMs = options.timeoutMs
        || (options.nonStreaming ? NONSTREAM_TIMEOUT_MS : CONNECT_TIMEOUT_MS)
