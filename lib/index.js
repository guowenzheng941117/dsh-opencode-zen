...
function log(ctx, level, msg) {
  try {
    const line = `[dsh-opencode-zen] ${msg}`
    const fn = ctx?.logger?.[level]
    if (typeof fn === 'function') { fn(line); return }
    // 某些宿主上下文不注入 logger——回退到 console，保证自愈过程可观测
...
class OpenCodeZenAdapter {
  ...
  async *stream(options) {
    ...
    const runOnce = async function* (reqBody) {
      ...
      return (marks.done || translator.sawFinishReason)
        ? 'clean'
        : (translator.openedBlocks ? 'aborted-mid' : 'aborted-empty')
    }
    ...
    while (status !== 'clean') {
      ...
      if (status === 'aborted-empty') {
        if (emptiesLeft-- <= 0) break
        log(this.ctx, 'warn', `upstream aborted an empty stream; retry #${EMPTY_STREAM_RETRIES - emptiesLeft} in ${wait}ms`)
      } else if (continuationsLeft > 0 && translator.continuable) {
        // 思考/正文半截：无缝续跑（同一块续流）
        continuationsLeft--
        log(this.ctx, 'warn', `upstream cut the stream mid-generation; auto-continue #${MAX_CONTINUATIONS - continuationsLeft} in ${wait}ms`)
        ...
        log(this.ctx, 'warn', `upstream cut the stream mid-tool-call; args quarantined, full retry in ${wait}ms`)
        await sleepOrAbort(wait, signal)
...
module.exports = { apply, inject, name, OpenCodeZenAdapter, StreamTranslator, buildContinuationBody, truncationBackoff, PROVIDER, MODELS, resolveApiKey }