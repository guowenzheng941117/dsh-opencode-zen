...
/**
 * 把内容块（含 tool-result 内层）解析成 OpenAI 多模态 part。
 * loadImage 由适配器注入（走宿主附件服务 readImageRequest）；
 * 解析失败降级为占位文本，绝不静默丢图。
 * read_image 等工具产出的图嵌在 tool-result 第二层——这里必须递归。
 */
async function multimodalParts(blocks, loadImage) {
  const parts = []
  let text = ''
    ...
    for (const b of bs || []) {
      if (b.type === 'text') text += b.text
        ...
        try {
          ...
          if (v?.data) {
            parts.push({ type: 'text', text: `[image ${v.width}x${v.height}px]` })
            parts.push({
              type: 'image_url',
              image_url: { url: `data:${v.mediaType};base64,${Buffer.from(v.data).toString('base64')}` },
            })
          } else {
...
/** 将 Harness 消息转成 OpenAI chat.completions 请求体（loadImage 提供时支持多模态） */
async function serializeMessages(messages, systemPrompt, loadImage) {
  ...
  for (const m of messages || []) {
    ...
    const text = flattenText(m.content)
    // 用户消息：有加载器且带图片块（含嵌套）→ 多模态 content 数组。
    ...
    if (role === 'user' && loadImage && toolResults.length === 0 && hasImageDeep(m.content)) {
      const parts = await multimodalParts(m.content, loadImage)
    ...
    for (const r of toolResults) {
      // 工具结果里的图（如 read_image 截图）→ 多模态 tool 消息
      if (loadImage && hasImageDeep(r.content)) {
        const parts = await multimodalParts(r.content, loadImage)
        wire.push({ role: 'tool', tool_call_id: r.toolCallId, content: parts.length ? parts : '(no output)' })
        continue
      }
...
module.exports = { apply, inject, name, OpenCodeZenAdapter, StreamTranslator, buildContinuationBody, truncationBackoff, serializeMessages, inputModalitiesOf, PROVIDER, MODELS, resolveApiKey }