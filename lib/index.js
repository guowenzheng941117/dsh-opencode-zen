...
function normalizeEntry(id, spec, staticEntry) {
  ...
  return {
    ...
    // 视觉：zen 免费档历史上对视觉会挂起（见历史 revert 提交），故不依据 models.dev 自动开启，
    // 仅当 models.json 显式声明 input 含 image 时才启用；imageSupported 仅作信息字段保留。
    input: Array.isArray(s?.input) ? s.input : ['text'],
...
async function ensureLiveModels(ctx) {
  ...
  _liveFetching = (async () => {
    ...
    let liveIds = null
    try {
      const res = await fetch(`${OPENCODE_BASE}/models`, { headers: { 'User-Agent': OPENCODE_UA } })
...
async function rawDescribeOnce(modelId, v, prompt, signal) {
  ...
  try {
    const response = await fetch(`${OPENCODE_BASE}/chat/completions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${resolveApiKey()}`, 'User-Agent': OPENCODE_UA },
      body: JSON.stringify({
        model: modelId,
 ...
 * 避免模型拿半份描述误判全局。
 */
async function describeViaStrips(ctx, modelId, v, signal) {
  ...
  for (const s of strips) {
    ...
    for (let a = 0; a < STRIP_DESC_ATTEMPTS && !txt; a++) {
      try {
        txt = await rawDescribeOnce(modelId, s, `${DESCRIBE_PROMPT} （注：这是完整图像自上而下的第 ${s.seq}/${s.count} 段条带，仅描述该段可见内容。）`, signal)
      } catch (e) {
        lastErr = e
