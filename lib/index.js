...
async function multimodalParts(blocks, loadImage) {
  ...
  for (const b of blocks || []) {
    ...
    // 其余块类型（reasoning 等）不出现在用户消息里，忽略
  }
  flushText()
  return parts
}

/** 将 Harness 消息转成 OpenAI chat.completions 请求体（loadImage 提供时支持用户消息多模态） */
async function serializeMessages(messages, systemPrompt, loadImage) {
  ...
  for (const m of messages || []) {
    ...
    const text = flattenText(m.content)
    // 用户消息：有加载器且确实带图片块 → 多模态 content 数组
    const hasImageBlock = Array.isArray(m.content) && m.content.some((b) => b.type === 'image')
    if (role === 'user' && loadImage && hasImageBlock) {
